...
int main() {
	...
	for (int i = -diff_x_; i < I_f.rows - diff_x_; i++) {
		for (int j = -diff_y_; j < I_f.cols - diff_y_; j++) {
			...
			{
				Vec3f V1 = mu * I2.at<Vec3f>(x1, y2) + (1 - mu) * I2.at<Vec3f>(x1, y1);
				Vec3f V2 = mu * I2.at<Vec3f>(x2, y2) + (1 - mu) * I2.at<Vec3f>(x2, y1);
...
Mat cal_affine(int ptl_x[], int ptl_y[], int ptr_x[], int ptr_y[], int number_of_points) {
	...
	for (int i = 0; i < number_of_points; i++) {
		...
		int pf1[6] = { ptl_x[i],ptl_y[i],1,0,0,0 };
		int pf2[6] = { 0,0,0,ptl_x[i],ptl_y[i],1 };
