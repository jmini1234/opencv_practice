...
int nearestNeighbor_dist(Mat& vec, vector<KeyPoint>& keypoints, Mat& descriptors) {
	double minDist = 1e6;
...
int secondnearestNeighbor(Mat& vec, vector<KeyPoint>& keypoints, Mat& descriptors, double minMax) {
	double minDist = 1e6;
...
void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1,
	...
	for (int i = 0; i < descriptors1.rows; i++) {
		...
		if (ratio_threshold) {
			...
			double second_nn = secondnearestNeighbor(desc1, keypoints2, descriptors2, nn_dist);
			if (nn_dist / second_nn > RATIO_THR)
				continue;
		}
