			...
			if (!strcmp(opt, "zero-padding")) {
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) { //if the pixel is not a border pixel
							kernel.at<G>(idx,0) = (float)(input.at<G>(i+x, j+y));
						}
						idx++;
					}
				}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>(idx++, 0) = (float)(input.at<G>(tempa, tempb));
					}
				}
			}
		...
		for (int j = 0; j < col; j++) {
			...
			if (!strcmp(opt, "zero-padding")) {
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						if ((i + x <= row - 1) && (i + x >= 0) && (j + y <= col - 1) && (j + y >= 0)) { //if the pixel is not a border pixel
							kernel.at<G>(idx, 0) = (float)(input.at<C>(i + x, j + y)[0]);
							kernel.at<G>(idx, 1) = (float)(input.at<C>(i + x, j + y)[1]);
							kernel.at<G>(idx, 2) = (float)(input.at<C>(i + x, j + y)[2]);
						}
						idx++;
					}
				}
				...
				for (int x = -n; x <= n; x++) { // for each kernel window
					for (int y = -n; y <= n; y++) {
						...
						kernel.at<G>(idx, 0) = (float)(input.at<C>(tempa, tempb)[0]);
						kernel.at<G>(idx, 1) = (float)(input.at<C>(tempa, tempb)[1]);
						kernel.at<G>(idx, 2) = (float)(input.at<C>(tempa, tempb)[2]);
						idx++;
					}
				}
			}
