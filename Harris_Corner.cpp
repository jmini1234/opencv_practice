...
//	Mat input = imread("checkerboard2.jpg", CV_LOAD_IMAGE_COLOR);
	...
	cornerHarris(input_gray, output, 2,3,0.03, BORDER_DEFAULT);
	...
	if (Subpixel)
	{
		...
		Size zeroZero = Size(-1, -1);

		cornerSubPix(input_gray, points, subPixWinSize, zeroZero, termcrit);
			...
			if (corner_mat.at<uchar>(i-radius, j-radius) == 1) {
				for (int x = -radius; x < radius; x++) {
					for (int y = -radius; y < radius; y++) {
						if (input_mirror.at<float>(i-x, j-y)>input.at<float>(i - radius, j - radius)) {
