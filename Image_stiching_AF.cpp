...
int main() {
	Mat input1 = imread("input1.jpg");
	Mat input2 = imread("input2.jpg");
	...
	double *ptl_x = new double[(int)srcPoints.size()];
	double *ptl_y = new double[(int)srcPoints.size()];
	double *ptr_x = new double[(int)dstPoints.size()];
	double *ptr_y = new double[(int)dstPoints.size()];
	...
	const float I1_col = input2.cols;
	const float I2_row = input1.rows;
	const float I2_col = input2.cols;

	// calculate affine Matrix A12, A21
	Mat A12 = cal_affine<float>(ptl_x, ptl_y, ptr_x, ptr_y, (int)srcPoints.size());
	Mat A21 = cal_affine<float>(ptr_x, ptr_y, ptl_x, ptl_y, (int)dstPoints.size());
