...
//	 split each channel(Y, U, V)
	...
	Mat Y_matching = channels_matching[0];
...
//	 trans 
	...
	G trans_func_mat_T[L] = { 0 };			// transfer function
	G trans_func_mat_G[L] = { 0 };			// transfer function
...
void compute_function(G *trans_func_T, G *trans_func_G, G*matching_func, float *CDF, float *CDF_G) {
	...
	for (int i = 0; i < L; i++) {
		...
		for (int j = 0; j < L; j++) {
			...
			// 만약 matching되는 값이 없으면 nearest value 
...
void hist_matching(Mat &input, Mat &matching, G *matching_func) {
	...
	for (int i = 0; i < input.rows; i++)
		for (int j = 0; j < input.cols; j++) {
			matching.at<G>(i,j) = matching_func[input.at<G>(i, j)]; // T(r) 즉 s 
		}
}
